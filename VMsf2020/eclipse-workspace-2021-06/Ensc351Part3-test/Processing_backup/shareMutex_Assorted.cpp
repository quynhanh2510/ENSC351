...
//define variables
shared_mutex mutexSocketInMap; //this mutex lock protects global variable
...
{
    shared_lock <shared_mutex> mapLk(mutexSocketInMap); //lock global mutex
//    if (socketMap[des].isOpen)
//        socketMap[des].isOpen = true; //change partner's status
...
{
    lock_guard<shared_mutex> mapLk(mutexSocketInMap); //lock global mutex
...
int myReadcond( int des, void * buf, int n, int min, int time, int timeout )
{
    ...
    if ( (socketMap.find(des) == socketMap.end()) || !(socketMap[des].isOpen) ) { //error case
        ...
        while(bytesRead < min && (unique_lock<shared_mutex> (mutexSocketInMap), socketMap[des].isOpen)) {
            unique_lock<shared_mutex> mapLk(mutexSocketInMap); //lock global mutex
            ...
            //data after being drained
            if(numBytesRead != -1)
                socketMap[socketMap[des].paired].numBytes -= numBytesRead;

            //notify that wcsReadcond needs more bytes to be written
            cv.notify_all();
        }
    }
