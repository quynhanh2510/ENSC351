...
void ReceiverY::purge()
{
    ...
    char purged_char;
    int bytesRead = myReadcond(mediumD, &purged_char, 1, 1, dSECS_PER_UNIT*TM_2CHAR, dSECS_PER_UNIT*TM_2CHAR);
    while (bytesRead != 0 && bytesRead != -1) {
        //Read and discard
        bytesRead = myReadcond(mediumD, &purged_char, 1, 1, dSECS_PER_UNIT*TM_2CHAR, dSECS_PER_UNIT*TM_2CHAR);
    }
}
