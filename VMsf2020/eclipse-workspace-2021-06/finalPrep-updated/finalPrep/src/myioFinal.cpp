...
void coutThreadFunc(void) {
    ...
    RetVal = myReadcond2(daSktPr[1], B, BSize, 17, 50);

    cout << "Output Line 2 - RetVal: " << RetVal << "  B: " << B << endl;
    myWrite(daSktPr[1], "wxyza", 5); // don't forget NUL termination char
    ...
    cout << endl;
    RetVal = myReadcond2(daSktPr[1], B, BSize, 5, 0);
    cout << "Output Line 3b- RetVal: " << RetVal;
    if (RetVal == -1)
        cout << " Error:  " << strerror(errno);
    else if (RetVal >= 0)
        cout << "  B: " << B;
    cout << endl;
    RetVal = myReadcond2(daSktPr[1], B, BSize, 5, 0);
    cout << "Output Line 3c- RetVal: " << RetVal;
    if (RetVal == -1)
        cout << " Error:  " << strerror(errno);
    else if (RetVal >= 0)
        cout << "  B: " << B;
	...
	RetVal = myReadcond2(daSktPr[1], B, BSize, 5, 0);
	cout << "Output Line 5b - RetVal: " << RetVal;
	    if (RetVal == -1)
	        cout << " Error:  " << strerror(errno) << endl;
	    else if (RetVal >= 0)
	        cout << "  B: " << B;
...
int main() { // debug launch configuration ensures gdb runs at FIFO priority 98 ...
	...
	try {		// ... realtime policy.
		...
		myWrite(daSktPr[0], "12321212121213", 14); // don't forget NUL termination character
		RetVal = myReadcond2(daSktPr[0], B2, 4, 4, 0);
	    cout << "Output Line 2b - RetVal: " << RetVal;
	    if (RetVal == -1)
	        cout << " Error:  " << strerror(errno);
	    else if (RetVal >= 0)
	        cout << "  B2: " << B2;
